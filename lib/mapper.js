...
function transformObject(source, schema, ctx) {
    ...
    for (const key in schema) {
        ...
        // 4. Plain nested object → map source[key]
        //    Advanced rule object ($allow, $from, $path, $date) → processAdvanced
        if (isObject(rule)) {
            const isAdvanced = Object.keys(rule).some(k => k.startsWith('$'));
            if (isAdvanced) {
                result[key] = processAdvanced(source, rule, ctx);
            } else {
                result[key] = mapper(source?.[key] ?? {}, rule, ctx);
            }
            continue;
        }
